...
shiftManagerProfile.post(
  ...
  async (req, res, next) => {
    ...
    try {
      mchQueryMod(
        `SELECT * 
FROM magodmis.ncprograms 
WHERE Machine = '${req.body.MachineName}' 
  AND Operation = '${req.body.Operation}' 
...
WHERE Machine = '${req.body.MachineName}' 
  AND Operation = '${req.body.Operation}' 
  AND (PStatus = 'Cutting' OR PStatus = 'Processing');
`,
        (err, data) => {
          if (err) logger.error(err);
          //console.log(data)
  ...
  async (req, res, next) => {
    ...
    try {
      misQueryMod(
        `SELECT * 
FROM magodmis.ncprograms 
WHERE Operation = '${req.body.Operation}' 
  AND (PStatus = 'Cutting' OR PStatus = 'Processing');
`,
        (err, data) => {
          if (err) logger.error(err);
